...
func TestTreeGenerate(t *testing.T) {
	...
	t.Log("levels", len(tree.Levels), "nodes", len(tree.Nodes))
...
func TestTreeGenerate_2(t *testing.T) {
	tree := NewTree()
	blocks := [][]byte{[]byte("foo"), []byte("bar")}
	if err := tree.Generate(blocks, NewSimpleHash()); err != nil {
		t.Fatal(err)
	}
	t.Log("levels", len(tree.Levels), "nodes", len(tree.Nodes), "root_hash", tree.Root().Hash)
}
func TestTreeGenerate_1(t *testing.T) {
	tree := NewTree()
	blocks := [][]byte{[]byte("foo")}
	if err := tree.Generate(blocks, NewSimpleHash()); err != nil {
		t.Fatal(err)
	}
	t.Log("levels", len(tree.Levels), "nodes", len(tree.Nodes), "root_hash", tree.Root().Hash)
